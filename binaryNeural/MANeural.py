...
def stockNeural(filename):
    datafile = 'data/neuralInput/'+filename+'/'+'input.xls'
    ...
    if not os.path.exists('data/neuralModels/'+filename):
        os.mkdir('data/neuralModels/'+filename)
    netfile = 'data/neuralModels/'+filename+'/'+'net.model'  # 构建的神经网络模型存储路径
    ...
    if os.path.exists('data/neuralModels/'+filename+'/net.model'):
        pass
    else:
    ...
    if not os.path.exists('data/modelPredict/' + filename):
        os.mkdir('data/modelPredict/' + filename)
    csvfile = file('data/modelPredict/' + filename+'/'+'resultPredict.csv', 'wb')
    ...
    predict_result = net.predict(test[:,1:29]).reshape(len(test))
    fpr, tpr, thresholds = roc_curve(test[:,29], predict_result, pos_label=1)
