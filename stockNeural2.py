...
def stockNeural(filename):
    datafile = 'neuralInput/'+filename+'/'+'input.xls'
    ...
    if not os.path.exists('neuralModels/'+filename):
        os.mkdir('neuralModels/'+filename)
    netfile = 'neuralModels/'+filename+'/'+'net.model'  # 构建的神经网络模型存储路径
    ...
    if os.path.exists('neuralModels/50ETF/net.model'):
        pass
    else:
    ...
    if not os.path.exists('modelPredict/' + filename):
        os.mkdir('modelPredict/' + filename)
    csvfile = file('modelPredict/' + filename+'/'+'resultPredict.csv', 'wb')
